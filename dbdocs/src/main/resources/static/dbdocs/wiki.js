  ...
  html += `<div class="wiki-page-header">
    <div class="wiki-page-title">
      <div style="flex:1;min-width:0">
        ...
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          <code style="font-size:11px;color:var(--text-muted);font-family:var(--mono,monospace);background:var(--surface2);border:1px solid var(--border);display:inline-block;padding:2px 8px;border-radius:3px">Table: ${escapeHtml(entity.tableName)}</code>
...
function buildEntityMetaPanel(meta) {
  ...
  const PRIORITY_LABELS = ['Domain', 'Submodule', 'PII', 'Type', 'Retention'];

  const priorityRows = PRIORITY_LABELS
    .map(l => populatedRows.find(r => r.label === l))
    .filter(Boolean);
  const restRows = populatedRows
    .filter(r => !PRIORITY_LABELS.includes(r.label))
    .sort((a, b) => a.label.localeCompare(b.label));

  // If no priority rows matched, fall back: show first 5 alphabetically, collapse the rest
  const visibleRows = priorityRows.length > 0 ? priorityRows : restRows.splice(0, 5);
  const collapsedRows = priorityRows.length > 0 ? restRows : restRows;
  ...
  if (collapsedRows.length > 0) {
    collapsedRows.forEach(({ label, value }) => {
      html += `<tr class="meta-extra-row" style="display:none;border-bottom:1px solid var(--border-light)">
        <td style="padding:8px 12px;color:var(--text-muted);font-size:12px;white-space:nowrap">${escapeHtml(label)}</td>
        <td style="padding:8px 12px">${metaValueHtml(label, value)}</td>
      </tr>`;
    });
    html += `<tr class="meta-seemore-row">
      <td colspan="2" style="padding:5px 12px;border-top:1px dashed var(--border)">
        <button onclick="
          const rows = this.closest('table').querySelectorAll('.meta-extra-row');
          const hidden = rows[0].style.display === 'none';
          rows.forEach(r => r.style.display = hidden ? '' : 'none');
          this.innerHTML = hidden
            ? 'See less &#9650;'
            : 'See ${collapsedRows.length} more &#9660;';
        " style="background:none;border:none;cursor:pointer;color:var(--accent);font-size:12px;font-weight:600;padding:2px 4px;letter-spacing:0.01em">
          See ${collapsedRows.length} more &#9660;
        </button>
      </td>
    </tr>`;
