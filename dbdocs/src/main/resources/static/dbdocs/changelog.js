    ...
    if (isExpanded && totalChanged > 0) {
      ...
      if (modified.length > 0) {
        ...
        modified.forEach(e => {
          ...
          const metadataChanges = e.metadataChanges || [];
          const addedCount   = [...fieldChanges, ...metadataChanges].filter(c => c.startsWith('+')).length;
          const deletedCount = [...fieldChanges, ...metadataChanges].filter(c => c.startsWith('-')).length;
          const docCount     = metadataChanges.length;
          item.innerHTML = `
            ...
            <span style="font-family:'JetBrains Mono',Consolas,monospace;font-size:12px;color:var(--text)">${escapeHtml(e.tableName)}</span>
            ${addedCount > 0   ? `<span style="font-size:10px;color:var(--success);font-weight:600">+${addedCount}</span>` : ''}
            ${deletedCount > 0 ? `<span style="font-size:10px;color:var(--danger);font-weight:600">-${deletedCount}</span>` : ''}
            ${docCount > 0 && addedCount === 0 && deletedCount === 0 ? `<span style="font-size:10px;color:var(--accent);font-weight:600">docs</span>` : ''}
          `;
          list.appendChild(item);
        });
...
function renderTableChangelog(entity) {
  ...
  relevant.forEach(({ entry, match, status }) => {
    ...
    const metadataChanges = match.metadataChanges || [];
    const allChanges      = [...fieldChanges, ...relationChanges, ...metadataChanges];
        ...
        <div class="cl-changes-box" style="margin-top:8px">
          ...
          <div class="cl-entity-list">
            ${allChanges.map(c => {
              ...
              const isMeta = c.includes('@');
              const color  = isMeta
                ? (isAdd ? '#7c6af5' : isRem ? '#b05bf5' : 'var(--accent)')
                : (isAdd ? 'var(--success)' : isRem ? 'var(--danger)' : 'var(--warning)');
              const bgCol  = isMeta
                ? (isAdd ? '#ede9fe' : isRem ? '#f3e8ff' : '#eef2ff')
                : (isAdd ? 'var(--success-light)' : isRem ? 'var(--danger-light)' : 'var(--warning-light)');
              ...
              return `<div class="cl-entity-item" style="cursor:default;gap:10px">
                ...
                ${isMeta ? `<span style="font-size:9px;color:${color};background:${bgCol};padding:1px 5px;border-radius:8px;font-weight:600;flex-shrink:0">docs</span>` : ''}
              </div>`;
            }).join('')}
          </div>
        </div>` : ''}
