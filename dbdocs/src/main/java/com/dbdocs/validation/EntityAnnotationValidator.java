...
import com.dbdocs.annotation.EntityMetadata;
import com.dbdocs.annotation.FieldMetadata;
import com.dbdocs.autoconfigure.DbvcsProperties;
import com.dbdocs.scanner.ClasspathEntityFinder;
 ...
 *       are read from {@code dbdocs.validation.*} in {@code application.properties}.</li>
 * </ol>
 *
...
public class EntityAnnotationValidator {
    ...
    public void validate(ClassLoader classLoader) {
        ...
        if (violations.isEmpty()) {
            System.out.println("[dbdocs] ✓ Annotation validation passed. All entities comply.");
            return;
        }
    ...
    private String buildReport(List<ViolationReport> violations, boolean failOnViolation) {
        ...
        if (failOnViolation) {
            sb.append("[dbdocs] ANNOTATION VALIDATION FAILED (rules via: ").append(mode).append(")\n");
        } else {
            sb.append("[dbdocs] ANNOTATION VALIDATION WARNING (rules via: ").append(mode).append(")\n");
        }
        
        sb.append("[dbdocs] ").append(violations.size())
          .append(violations.size() == 1 ? " entity" : " entities")
          .append(" missing required annotations:\n");
        sb.append("[dbdocs] ─────────────────────────────────────────────────────\n");

        for (ViolationReport v : violations) {
            sb.append("[dbdocs]  ✗ ").append(v.className()).append("\n");
            
            // Entity-level missing annotations
            for (String ann : v.missingAnnotations()) {
                sb.append("[dbdocs]      missing: @").append(ann).append("\n");
            }
            ...
            if (v.fieldViolations() != null && !v.fieldViolations().isEmpty()) {
                for (FieldViolationReport fv : v.fieldViolations()) {
                    if ("<entity>".equals(fv.fieldName())) {
                        sb.append("[dbdocs]      @EntityMetadata missing required attributes:\n");
                    } else {
                        sb.append("[dbdocs]      field '").append(fv.fieldName()).append("' @FieldMetadata missing required attributes:\n");
                    }
                    for (String ann : fv.missingAnnotations()) {
                        sb.append("[dbdocs]        missing attribute: ").append(ann).append("\n");
                    }
                }
            }
        }
        ...
        if (!failOnViolation) {
            sb.append("[dbdocs] Continuing execution with warnings (fail-on-violation=false).\n");
            sb.append("[dbdocs] To make this a startup error, set dbdocs.validation.fail-on-violation=true.\n");
        }
