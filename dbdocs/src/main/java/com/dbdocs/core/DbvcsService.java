...
import com.dbdocs.scanner.EntityScanner;
import com.dbdocs.versioning.SchemaDiffer;
...
public class DbvcsService {
    ...
    public void initialize(ClassLoader classLoader) {
        ...
        if (differ.hasChanged(previous, entities)) {
            ...
            System.out.println("[dbdocs] New schema version saved: schema-" + nextVersion + ".json"
                    + " (" + entities.size() + " entities)");
        } else {
            activeSnapshot = previous;
            System.out.println("[dbdocs] Schema unchanged. Current version: " + latest);
        }
    }
